...
function initTouchAnimations(){
  const touchables = document.querySelectorAll('.btn,.header-cta,.gate-btn,.utility-btn,.menu-btn,.device-head a,.device-list a,.board-row,.category-card,.info-card,.step-card,.social-links a,.footer-socials a');
  touchables.forEach(el => {
    ...
    el.addEventListener('animationend', e => {
      if(e.animationName === 'touchTap') el.classList.remove('tap-animate');
    });
  });
}
